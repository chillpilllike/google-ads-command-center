...
from app.models import GoogleAdsAccount, GoogleAdsKeywordCandidate, User
from app.services.background_jobs import create_background_job, mark_job_dispatch_failed, save_job_message_id
from app.services.google_ads_keyword_csv import keyword_bank_csv_filename, keyword_candidates_csv_chunk
...
def _keyword_url(
    *,
    ...
    return "/keywords?" + urlencode(params)


def _keyword_export_url(
    *,
    account_id: Optional[int],
    q: str = "",
    quality: str = "",
    ...
    if quality:
        params["quality"] = quality
    return "/keywords/export" + (("?" + urlencode(params)) if params else "")


def _candidate_order() -> list:
    return [
        desc(GoogleAdsKeywordCandidate.score),
        desc(GoogleAdsKeywordCandidate.conversions),
        desc(GoogleAdsKeywordCandidate.all_conversions),
        desc(GoogleAdsKeywordCandidate.clicks),
        desc(GoogleAdsKeywordCandidate.last_seen_at),
...
def _candidate_filters(
    *,
    ...
    filters = [GoogleAdsKeywordCandidate.account_id == account_id]
    q = str(q or "").strip()
    quality = str(quality or "").strip().lower()
    if q:
        filters.append(GoogleAdsKeywordCandidate.keyword.ilike(f"%{q}%"))
    if quality in QUALITY_LABELS:
        filters.append(GoogleAdsKeywordCandidate.quality_label == quality)
    return filters


async def _stream_keyword_candidates_csv(
    *,
    account_id: int,
    q: str = "",
    quality: str = "",
    ...
    yield keyword_candidates_csv_chunk([], include_header=True)
    offset = 0
    ...
    async with AsyncSessionLocal() as stream_session:
        while True:
            ...
            if selected_ids:
                filters.append(GoogleAdsKeywordCandidate.id.in_(selected_ids))
            rows = (
            ...
            if not rows:
                break
            yield keyword_candidates_csv_chunk(rows)
            offset += len(rows)


@router.get("/keywords", response_class=HTMLResponse)
async def keyword_bank_page(
    request: Request,
    ...
    summary_subq = (
        select(
            ...
            func.max(GoogleAdsKeywordCandidate.last_seen_at).label("last_seen_at"),
    ...
    account_summaries = [
        {
            "account": row[0],
            "keyword_count": int(row[1] or 0),
            "last_seen_at": row[2],
            "last_pulled_at": row[3],
            "url": _keyword_url(account_id=row[0].id, per_page=per_page, q=q, quality=quality),
        }
    ...
    if selected_account is not None:
        ...
        total = int(await session.scalar(select(func.count(GoogleAdsKeywordCandidate.id)).where(*filters)) or 0)
        ...
        copy_keywords = list(
            (
                await session.scalars(
                    select(GoogleAdsKeywordCandidate.keyword)
                    .where(*filters)
                    .order_by(*_candidate_order())
        ...
        count_rows = (
            await session.execute(
                select(GoogleAdsKeywordCandidate.quality_label, func.count(GoogleAdsKeywordCandidate.id))
                .where(GoogleAdsKeywordCandidate.account_id == selected_account.id)
                .group_by(GoogleAdsKeywordCandidate.quality_label)
            )
        ).all()
        quality_counts = {str(row[0] or ""): int(row[1] or 0) for row in count_rows}

    page_keywords = [row.keyword for row in keyword_rows]
    return templates.TemplateResponse(
        "keywords.html",
        {
            ...
            "export_filtered_url": _keyword_export_url(
                account_id=selected_account.id if selected_account else None,
                q=q,
                quality=quality,
            ),
...
async def export_keyword_bank_csv(
    account_id: int,
    q: str = "",
    quality: str = "",
    ...
    selected_ids = [int(item) for item in (selected_ids or []) if int(item) > 0]
    filename = keyword_bank_csv_filename(account, selected=bool(selected_ids))
    response = StreamingResponse(
        _stream_keyword_candidates_csv(
...
async def sync_keyword_bank(
    ...
    label = (
        f"{'All-time' if pull_mode == 'all_time' else str(days) + 'd'} keyword pull: {label_target}"
    )
    job = await create_background_job(
        session,
        job_type="google_ads_keyword_manual_sync",
        label=label,
    ...
    try:
        from app.tasks import sync_google_ads_keyword_pull

        message = sync_google_ads_keyword_pull.send(
            pull_mode,
            days,
            job.id,
    ...
    return RedirectResponse(
        f"/keywords?account_id={target_account_id}&sync_job_id={job.id}&sync_scope={sync_scope}",
