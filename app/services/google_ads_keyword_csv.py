...
from app.models import GoogleAdsAccount, GoogleAdsKeywordCandidate


KEYWORD_BANK_CSV_COLUMNS = [
    "Keyword",
    "Normalized keyword",
    "Match type",
    "Quality",
...
def keyword_candidate_csv_row(candidate: GoogleAdsKeywordCandidate) -> list[object]:
    return [
        candidate.keyword or "",
        candidate.normalized_keyword or "",
...
def keyword_candidates_csv_chunk(
    candidates: Iterable[GoogleAdsKeywordCandidate],
    *,
    ...
    if include_header:
        writer.writerow(KEYWORD_BANK_CSV_COLUMNS)
    for candidate in candidates:
        writer.writerow(keyword_candidate_csv_row(candidate))
    return output.getvalue()


def keyword_bank_csv_filename(account: GoogleAdsAccount, *, selected: bool = False) -> str:
    ...
    return f"keyword-bank-{slug}{suffix}.csv"
