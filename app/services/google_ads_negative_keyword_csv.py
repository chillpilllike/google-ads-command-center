...
from app.models import GoogleAdsAccount, GoogleAdsNegativeKeywordCandidate
from app.services.google_ads_negative_keyword_bank import negative_match_text


NEGATIVE_KEYWORD_BANK_CSV_COLUMNS = [
    "Negative keyword",
    "Google Ads match text",
    "Match type",
    "Scope",
    "Campaign ID",
    "Campaign",
...
def negative_keyword_candidate_csv_row(candidate: GoogleAdsNegativeKeywordCandidate) -> list[object]:
    return [
        candidate.keyword or "",
        negative_match_text(candidate.keyword or "", candidate.match_type or "exact"),
        candidate.match_type or "",
        candidate.scope_level or "",
        _number(candidate.campaign_id),
        candidate.campaign_name or "",
        candidate.reason_label or "",
        _number(candidate.confidence),
        candidate.guard_status or "",
        ...
        _joined(candidate.guard_reasons),
        _joined(candidate.ad_group_names),
...
def negative_keyword_candidates_csv_chunk(
    candidates: Iterable[GoogleAdsNegativeKeywordCandidate],
    *,
    ...
    if include_header:
        writer.writerow(NEGATIVE_KEYWORD_BANK_CSV_COLUMNS)
    for candidate in candidates:
        writer.writerow(negative_keyword_candidate_csv_row(candidate))
    return output.getvalue()


def negative_keyword_bank_csv_filename(account: GoogleAdsAccount, *, selected: bool = False) -> str:
    ...
    return f"negative-keyword-bank-{slug}{suffix}.csv"
